...
export default function HospitalTable({ hospitals, onEditHospital, onAddDicom }) {

  // Helper function to get contact by type
  const getContactByType = (contacts, type) => {
    return contacts?.find(c => c.contactType === type);
  };
  ...
  const columns = [
    {
      ...
      isResizable: true,
      onRender: (item) => (
          // Plain text - removed clickable link
    ...
    },
    {
      ...
      maxWidth: 250,
      onRender: (item) => {
        const primaryContact = getContactByType(item.contacts, 0);
    ...
    },
    {
      ...
      maxWidth: 250,
      onRender: (item) => {
        const secondaryContact = getContactByType(item.contacts, 1);
      ...
      onRender: (item) => (
          <Stack horizontal tokens={{ childrenGap: 8 }}>
            ...
            <IconButton
                ...
                onClick={() => onAddDicom && onAddDicom(item)}
            />
